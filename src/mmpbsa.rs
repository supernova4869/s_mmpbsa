...
use ndarray::{array, Array, ArrayBase, Dim, Ix, NdIndex, OwnedRepr};
use fs::File;
use std::cmp::max;
use std::io::Write;
...
fn gen_qrv(mdp: &String, ndx: &Index, wd: &Path, receptor_grp: usize, ligand_grp: usize, qrv: &str, rad_type: i32, rad_lj0: f64) {
    ...
    let locator = get_md_locators(&mdp, &re)[0];
    ...
    let mol_num: Vec<i32> = get_md_params(&mdp, &re);
    let mol_num = mol_num[0];
    let re = Regex::new(r"\s*moltype\s*=\s*(.+?)\s*").unwrap();
    let Imol: Vec<i32> = get_md_params(&mdp, &re);
    let re = Regex::new(r"\s*#molecules\s*=\s*(.+?)\s*").unwrap();
    let Nmol: Vec<i32> = get_md_params(&mdp, &re);
    let re = Regex::new(r"atom \((.+)\):").unwrap();
    let Natm: Vec<i32> = get_md_params(&mdp, &re);
    println!("{}", Natm.iter().max().unwrap());
    // Name = Vector{String}()
    // Natm = Vector{Int32}()
...
fn get_md_locators(strings: &Vec<&str>, re: &Regex) -> Vec<usize> {
...
fn get_md_params<T: Debug + FromStr>(strings: &Vec<&str>, re: &Regex) -> Vec<T> where <T as FromStr>::Err: Debug {
