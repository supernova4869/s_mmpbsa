...
It may be generated by previous version of s_mmpbsa."#, &env!("CARGO_PKG_VERSION"), &sm_path);
            exit(0);
        };
    ...
    if Path::new(&tpr).is_file() {
        ...
        let tpr_desc = gmx::load_tpr(&tpr, &settings);
        fun_para_basic::set_para_basic_tpr(tpr_desc, &trj, &tpr, &ndx, &config, &mut settings);
    } else {
...
fn env_check() -> Settings {
    ...
    let settings = match get_settings_in_use() {
        Some(settings_file) => {
